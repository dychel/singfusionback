...
public class FormalitesController {
    ...
    public ResponseEntity<?> createFormalites(@RequestBody FormalitesDTO formalitesDTO) {
        Formalites formaliteExists = formalitesService.findFormalitesByIdUsers(formalitesDTO.getUserId());
        if (formaliteExists!=null)
            throw new ApiRequestException("Deja effectue cette etape");
        formalitesService.saveFormalites(formalitesDTO);
        Formalites formaliteJustSave = formalitesService.findFormalitesByIdUsers(formalitesDTO.getUserId());
        return new ResponseEntity<>(new ResponseMessage("ok", "Formalites "+ formalitesDTO.getTitre()+ " Créé avec succès", formaliteJustSave),
                HttpStatus.OK);
    }
